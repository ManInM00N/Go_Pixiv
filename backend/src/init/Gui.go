...
func Download_By_Rank(text, Type string, callEvent func(name string, data ...interface{})) {
	...
	for i := int64(1); i < int64(3); i++ {
		...
		TaskPool.Add(func() {
			...
			if WaitingTasks > 0 {
				NowTaskMsg = "There are " + fmt.Sprintf("%d", WaitingTasks) + " waiting tasks"
			} else {
				NowTaskMsg = "There is no tasks waiting"
			}
			callEvent("UpdateQueueNow", QueueTaskMsg)
			NowTaskMsg = op.RankDate + " " + op.Rank + " are downloading:"
			callEvent("UpdateTaskNow", NowTaskMsg)
			...
			P.Wait()
			NowTaskMsg = "Now Recheck " + op.RankDate + " " + op.Rank
			callEvent("UpdateTaskNow", NowTaskMsg)
			println(len(c), " ", satisfy)
			for len(c) > 0 {
				if IsClosed {
